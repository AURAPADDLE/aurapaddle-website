...
async function createOrderIdentity(requestId,attribution,customerEmail,items){return store.mutate(state=>reserveCheckoutIdentity(state,{requestId,attribution,customerEmail,items}))}
...
async function requestBalance(req,res){
  ...
  if(order.initialPaymentStatus!=="paid"||order.paymentStage==="paid_in_full"||["cancelled","refunded","dispatched"].includes(order.fulfilmentStatus))return send(res,409,{error:"This order is not eligible for a remaining-balance request."});
