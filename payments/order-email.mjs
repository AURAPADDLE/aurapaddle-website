...
export function adminOrderEmailContent(entry,{catalog,siteUrl}){
  ...
  const text=["New paid AURA PADDLE order", "",`Order: ${entry.orderNumber}`,`Customer: ${entry.customerName||"Not provided"}`,`Email: ${entry.customerEmail||"Not provided"}`,`Phone: ${entry.customerPhone||"Not provided"}`,"",...itemText(details),"",`${paidInFull?"Full payment including shipping":"Initial payment"}: ${money(entry.amountTotal,entry.currency)}`,`Remaining product balance: ${money(remaining,entry.currency)}`,`Delivery: ${shipping}`,`Estimated dispatch: ${dispatch}`,"",`Order status: ${statusUrl}`,`Stripe payment: ${stripeUrl}`].join("\n");
