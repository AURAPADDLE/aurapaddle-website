...
function deliveryAddressLines(entry){
  const address=entry.deliveryAddress||{};
  return [address.line1,address.line2,[address.city,address.state,address.postalCode].filter(Boolean).join(" "),address.country].filter(Boolean);
}
...
function shippingCopy(entry){
  const paidInFull=entry.paymentStage==="paid_in_full";
  ...
  return `${entry.shippingLabel||"Selected delivery region"} — ${money(entry.shippingAmount,entry.currency)}, ${paidInFull?"included in today's payment":"payable with the remaining product balance before dispatch"}.`;
}
...
export function customerOrderEmailContent(entry,{catalog,siteUrl}){
  ...
  const paidInFull=entry.paymentStage==="paid_in_full",remaining=paidInFull?0:Number(entry.amountTotal||0),dispatch=dispatchCopy(details),shipping=shippingCopy(entry);
  const subject=`Order confirmed — ${entry.orderNumber} | AURA PADDLE`;
  if(paidInFull){
    const text=[`Hi ${firstName(entry.customerName)},` ,"",`Your full payment for AURA PADDLE order ${entry.orderNumber} has been received.`,"",...itemText(details),"",`Total paid, including shipping: ${money(entry.amountTotal,entry.currency)}`,`Delivery: ${shipping}`,`Dispatch: ${dispatch}`,"","View your secure order status:",statusUrl,"","No further product balance is due. Delivery transit time is additional. See our returns policy; Australian Consumer Law rights are not limited."].join("\n");
    const body=`<p>Hi ${escapeHtml(firstName(entry.customerName))},</p><p>Your full payment has been received and your order is confirmed.</p><div style="background:#eef5f5;padding:18px 20px;border-radius:8px;margin:22px 0"><strong>${escapeHtml(entry.orderNumber)}</strong><ul style="padding-left:22px;margin:12px 0">${itemHtml(details)}</ul><p><strong>Total paid, including shipping:</strong> ${escapeHtml(money(entry.amountTotal,entry.currency))}</p><p><strong>Delivery:</strong> ${escapeHtml(shipping)}</p><p><strong>Dispatch:</strong> ${escapeHtml(dispatch)}</p></div><p><a href="${escapeHtml(statusUrl)}" style="display:inline-block;background:#f15b45;color:#fff;text-decoration:none;padding:14px 22px;border-radius:6px;font-weight:700">View order status</a></p><p>No further product balance is due. Delivery transit time is additional. Australian Consumer Law rights are not limited.</p>`;
...
export function adminOrderEmailContent(entry,{catalog,siteUrl}){
  ...
  const addressLines=deliveryAddressLines(entry),addressText=addressLines.length?addressLines.join("\n"):"Not supplied";
  const text=["New paid AURA PADDLE order", "",`Order: ${entry.orderNumber}`,`Customer: ${entry.customerName||"Not provided"}`,`Email: ${entry.customerEmail||"Not provided"}`,`Phone: ${entry.customerPhone||"Not provided"}`,`Ship to: ${addressText}`,"",...itemText(details),"",`${paidInFull?"Full payment including shipping":"Initial payment"}: ${money(entry.amountTotal,entry.currency)}`,`Remaining product balance: ${money(remaining,entry.currency)}`,`Delivery: ${shipping}`,`Estimated dispatch: ${dispatch}`,"",`Order status: ${statusUrl}`,`Stripe payment: ${stripeUrl}`].join("\n");
  const body=`<div style="background:#eef5f5;padding:18px 20px;border-radius:8px;margin:0 0 22px"><p style="margin:6px 0"><strong>Customer:</strong> ${escapeHtml(entry.customerName||"Not provided")}</p><p style="margin:6px 0"><strong>Email:</strong> ${escapeHtml(entry.customerEmail||"Not provided")}</p><p style="margin:6px 0"><strong>Phone:</strong> ${escapeHtml(entry.customerPhone||"Not provided")}</p><p style="margin:6px 0"><strong>Ship to:</strong><br>${addressLines.length?addressLines.map(escapeHtml).join("<br>"):"Not supplied"}</p></div><ul style="padding-left:22px">${itemHtml(details)}</ul><p><strong>${paidInFull?"Full payment including shipping":"Initial payment received"}:</strong> ${escapeHtml(money(entry.amountTotal,entry.currency))}</p><p><strong>Remaining product balance:</strong> ${escapeHtml(money(remaining,entry.currency))}</p><p><strong>Delivery:</strong> ${escapeHtml(shipping)}</p><p><strong>Estimated dispatch:</strong> ${escapeHtml(dispatch)}</p><p style="margin:26px 0"><a href="${escapeHtml(stripeUrl)}" style="display:inline-block;background:#102b38;color:#fff;text-decoration:none;padding:13px 20px;border-radius:6px;font-weight:700;margin-right:8px">Open Stripe payment</a> <a href="${escapeHtml(statusUrl)}" style="display:inline-block;color:#102b38;text-decoration:none;padding:12px 18px;border:1px solid #b8c7cc;border-radius:6px;font-weight:700">View order status</a></p>`;
